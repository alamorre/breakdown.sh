import { deleteNodeForActor, updateNodeForActor } from '@/lib/breakdown-service/nodes';
import { updateNodeSchema } from '@/lib/breakdown-service/schemas';
...
export async function PATCH(request: Request, { params }: { params: Promise<{ nodeId: string }> }) {
  ...
  return withHeadlessJson(
    request,
    'graphs:write',
    updateNodeSchema.omit({ nodeId: true }),
    (actor, body) => updateNodeForActor(actor, { nodeId, ...body }),
  );
}
