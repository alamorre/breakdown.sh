...
import { runNodeSchema } from '@/lib/thesis-service/schemas';
...
export async function POST(request: Request, { params }: { params: Promise<{ nodeId: string }> }) {
  ...
  return withHeadlessJson(request, 'runs:execute', runNodeSchema.omit({ nodeId: true }), (actor, body) =>
    runNodeForActor(actor, { nodeId, ...body }),
  );
}
