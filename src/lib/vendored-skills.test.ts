...
async function readVendoredManifest() {
  return JSON.parse(
    await readFile(join(pluginSkillsRoot, 'VENDORED_SKILLS.json'), 'utf8'),
...
describe('vendored Matt Pocock skill pack', () => {
  it('pins provenance and authenticates every vendored plugin file', async () => {
    ...
    expect(
      await readFile(join(pluginSkillsRoot, manifest.upstream.license_file), 'utf8'),
    ).toContain('Copyright (c) 2026 Matt Pocock');

    for (const skill of manifest.skills) {
      const skillRoot = join(pluginSkillsRoot, skill.name);
  ...
  it('routes ask-matt only to bundled skills and labels excluded upstream flows unavailable', async () => {
    const askMatt = await readFile(join(pluginSkillsRoot, 'ask-matt', 'SKILL.md'), 'utf8');
  ...
  it('includes every vendored skill in both deterministic standalone archives', async () => {
    ...
    const result = await buildSkillsArtifacts({
      ...
      vendoredSkillsRoot: pluginSkillsRoot,
    });
