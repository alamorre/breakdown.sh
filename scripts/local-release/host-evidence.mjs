...
const GUIDED_HOST_STAGE_PROCEDURES = Object.freeze([
  stageProcedure('install', 0, {
    ...
    prompt_or_action:
      'Invoke setup-breakdown for the explicit qualification-project root and exact host surface/version. The qualification harness has already inspected and pinned the kit-bound CLI and five canonical skills. The reviewed install-exact-candidate manifest operation is the already-granted approval for the fixed installer and disposable probe, so do not request another interactive approval or enumerate the pinned manifest. Execute the two fixed commands immediately in the prompted order, then report the full preflight result. Do not provide a host-evidence index.',
