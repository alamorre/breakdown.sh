...
## Qualify real Agent Hosts without an interim human step

`local-host-evidence-capture.yml` conducts and independently reviews the complete 13-stage journey
on GitHub-hosted Linux and macOS runners. It downloads the exact platform-qualified candidate,
uses a short-lived GitHub Actions token for GitHub Copilot CLI, and creates no package, tag,
release, support table, or other publication output.

Dispatch it with the immutable artifacts produced by the one platform-qualification run:
...
gh api "repos/alamorre/breakdown.sh/actions/runs/$capture_run_id/artifacts" \
  --jq '.artifacts[] | select(.name | startswith("breakdown-host-evidence-")) | {id, name, digest, expires_at}'
```

Each execution row has its own Agent Host session. Each fresh review job uses a different session
and the opposite model/provider family, reads only retained sanitized evidence, applies every
rubric gate, and uploads one 90-day qualified row. Execution and review identities remain in the
row; neither may be represented as human. The checked-in authorization manifest is the complete
authority for fixture-only effects, and hostile fixture content cannot expand it.

Before signing the source tag, bind those unchanged passing rows to the candidate source, generate
the support table, and attest the exact pre-release index:
...
gh api "repos/alamorre/breakdown.sh/actions/runs/$support_run_id/artifacts" \
  --jq '.artifacts[] | select(.name | startswith("breakdown-host-support-")) | {id, name, digest, expires_at}'
```

The resulting support artifact contains the candidate/source-bound index, its Sigstore bundle,
and generated JSON/Markdown. Stable publication later verifies this unchanged set and writes an
explicit binding from its digest and attestation to the protected signed tag. Qualification never
publishes packages, tags, releases, support claims, or external content. Windows and all unindexed
host tuples remain Unsupported or Compatible according to the checked-in classifications.
...
Retain the pre-release host-support artifact created before this tag. Do not re-index or alter its
rows after signing; the stable workflow verifies its candidate-source attestation and binds its
unchanged digest to the protected tag.
