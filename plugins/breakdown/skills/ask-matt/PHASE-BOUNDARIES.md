...
> Breakdown availability: this file preserves the upstream decision framework, but the upstream
> `handoff` skill is not bundled. Treat `/clear`, `/compact`, handoff, and subagent delegation as
> host-native options only when the active host provides them. Otherwise continue in the current
> session or write a plain Markdown handoff with the user's approval.
